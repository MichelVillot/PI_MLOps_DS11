...
peliculas = pickle.load(open(r"C:\Users\miche\OneDrive\Escritorio\Proyecto_MLOps\Notebooks\lista_peliculas.pkl", "rb"))
similaridad = pickle.load(open(r"C:\Users\miche\OneDrive\Escritorio\Proyecto_MLOps\Notebooks\similaridad.pkl", "rb"))
...
def votos_titulo(titulo_de_la_filmacion: str):
    ...
    titulo_de_la_filmacion = titulo_de_la_filmacion.title()
    encontrado = False
    diccionario = dict(df["title"])
    ind = []
    
    for indice in diccionario:
        if titulo_de_la_filmacion in diccionario[indice]:
            encontrado = True
            ind.append(indice)
            break
        
    anio = df.loc[ind[0], ["release_year"]][0]
    votos = df.loc[ind[0], ["vote_count"]][0]
    promedio_votos = df.loc[ind[0], ["vote_average"]][0]
    
    anio = int(anio)
    votos = int(votos)
    promedio_votos = round(promedio_votos, 10)
...
def get_director(nombre_director: str):
    ...
    # pelicula = {"Titulo de la Pelicula": titulo, "Fecha de Lanzamiento": fecha_lanzamiento, "Retorno Individual": retorno_individual, "Costo de la Pelicula": costo, "Ganancia de la Pelicula": ganancia}
    # df_peli = pd.DataFrame(pelicula)
    
    # df_dict = df_peli.to_dict(orient="records")
